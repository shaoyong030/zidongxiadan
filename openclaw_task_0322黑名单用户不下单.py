...
def run_pdd_to_taobao_task(stop_event):
    ...
    dlog("\n" + "="*50)
    dlog("🎬 [系统启动] 全链路闭环 V68.0 (新增精准买家级风控+数量自动同步版)")
    dlog("="*50)
        ...
        while True:
            ...
            for row in order_rows:
                ...
                if "此订单需要平台人工审核，请暂时不要发货" in txt:
                    ...
                    # 兜底：如果列索引失效，用正则抓取类似 R*** 的打码名称
                    if not buyer_name:
                        match = re.search(r'([A-Za-z0-9\u4e00-\u9fa5_]+\*\*\*)', txt)
                        if match: buyer_name = match.group(1)
                    
                    if buyer_name:
                        risky_buyers.add(buyer_name)

            if risky_buyers:
                dlog(f"   ∟ [动态风控] ⚠️ 发现高危买家名单: {risky_buyers}，将精准拦截其新单！")
            ...
            for index, row in enumerate(order_rows):
                ...
                if "绿色" in txt and "待发货" in txt:
                    ...
                    if current_buyer in risky_buyers:
                        dlog(f"     ∟ [跳过] 🛡️ 触发防守机制：买家【{current_buyer}】处于平台审核名单，暂停垫资。")
                        continue 
                    ...
                    if sn in merged_sns:
                        ...
                        if pdd_id in mapping_dict:
                            ...
                            dlog(f"     ∟ 匹配成功！准备下单，需拍数量: {extracted_qty}。")
                            ...
                            if qty_input.is_visible():
                                ...
                                for _ in range(current_quantity - 1):
                                    ...
                                    if plus_btn.is_visible():
                                        ...
                                        time.sleep(0.5)
                            dlog(f"   ∟ [成功] 淘宝下单数量已修正为 {current_quantity}")
                        except Exception as e:
                            dlog(f"   ∟ [警告] 数量调整可能失败，如果淘宝页面变动请留意: {e}")
