...
def run_pdd_to_taobao_task(stop_event):
    ...
    with sync_playwright() as p:
        try:
            ...
            dlog(f"✅ [初始化] 浏览器分身接管成功。")
        except Exception as e:
            dlog(f"❌ [初始化] 浏览器连接失败: {e}")
            return "\n".join(log_buffer), ""
        ...
        while True:
            ...
            target_unit, action, tb_id, current_sn, target_tb_link = None, "", "", "", ""

            for index, row in enumerate(order_rows):
                try:
                    txt = row.locator('xpath=..').inner_text().replace('\n', ' ')
                except:
                ...
                if "绿色" in txt and "待发货" in txt:
                    ...
                    if tb_match:
                        ...
                        if tid not in already_checked_logistics:
                            ...
                            target_unit, action, tb_id, current_sn = row, "LOGISTICS", tid, sn; break
                elif "合并" in txt:
                    ...
                    if merged_sns is None:
                        ...
                        target_unit = "REFRESH_DOM"; break 
                    
                    if sn in merged_sns:
                        dlog(f"     ∟ 命中[逻辑3]: 该单在合并名单中，准备添加备注")
                        target_unit, action, current_sn = row, "MARK_MERGE", sn; break
                    else:
                        dlog(f"     ∟ 命中[逻辑4]: 纯新白单！检查 Excel 映射...")
                        if pdd_id in mapping_dict:
                            target_tb_link = mapping_dict[pdd_id]
                            dlog(f"     ∟ 匹配成功！准备下单。")
                            target_unit, action, current_sn = row, "PLACE_ORDER", sn; break
                        else:
                            dlog(f"     ∟ 匹配失败！Excel 里没有此 ID。")
                            processed_sns.add(sn); continue

            if target_unit == "REFRESH_DOM": continue 
            if not target_unit: break
            ...
            if action == "PLACE_ORDER":
                ...
                try:
                    target_unit.locator('a:has-text("查看")').first.click(force=True); time.sleep(1)
                    phone_btn = target_unit.locator('a:has-text("查看手机号")')
                    if phone_btn.count() > 0: phone_btn.first.click(force=True); time.sleep(1.5)

                    copy_btn = target_unit.locator('a:has-text("复制完整信息")')
                    if copy_btn.count() > 0:
                        pyperclip.copy("") 
                    ...
                    try:
                        ...
                        if "请选择" in region_text:
                            ...
                            for part in addr_parts[:4]: 
                                ...
                                try: address_frame.locator(f'li:has-text("{part}")').first.click(timeout=800); time.sleep(0.5)
                                except:
                                    try:
                                        short = part.replace("省","").replace("市","").replace("区","").replace("县","").replace("自治区","")
                                        if short: address_frame.locator(f'li:has-text("{short}")').first.click(timeout=800); time.sleep(0.5)
                                    except: pass 
                    ...
                    if tb_order_id in ["下单未成功", "单号提取失败"]:
                        ...
                        time.sleep(1)
                        try:
                            remark_btn = target_unit.locator('a:has-text("添加备注"), a:has-text("修改备注")').first
            ...
            # ==========================================
                            ...
                            if logistics_btn.count() > 0:
                                ...
                                if popover.is_visible():
                                    ...
                                    if comp and sn and not sn.startswith("6000"):
                                        ...
                                        ship_btn = target_unit.locator('div[data-tracking-click-viewid="ele_single_order_ship_button"] button').first
                                        if ship_btn.count() > 0:
                                            ship_btn.scroll_into_view_if_needed(); ship_btn.click(force=True); time.sleep(2.5)
                                            pdd_page.fill('#trackingNumber input', sn)
                                            pdd_page.locator('#shippingId input').fill(comp[:2])
                                            time.sleep(1); pdd_page.keyboard.press("Enter"); time.sleep(1)
                                            pdd_page.locator('button[data-tracking-click-viewid="ele_confirm_shipment_shared"]').first.click(force=True)
                                            for i in range(16):
                                                time.sleep(0.5)
                                                result = pdd_page.evaluate('''() => {
                                                    let btns = document.querySelectorAll('button[data-tracking-click-viewid="makesure"]');
                                                    if (btns.length > 0) { btns[btns.length - 1].click(); return "CLICKED"; } return "WAITING";
                                                }''')
                                                if result == "CLICKED": break
                                            dlog(f"   ∟ [完成] 订单物流回填完毕。")
                                            processed_sns.add(current_sn); stats["logistics_updated"] += 1; time.sleep(2)
                                        else: 
                                            dlog("   ∟ [跳过] 找不到拼多多的发货按钮。")
                ...
                target_unit.locator('a:has-text("备注")').first.click(force=True); time.sleep(1.5)
